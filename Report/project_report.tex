...
\begin{document}
	...
	\section{Implementation and Project Design}
		...
		So first off we need to build our kernel that will allow us to shift the signal. To do this we chose to use the Continuous Fourier transform on a sinusoid function that was defined in the frequency domain.
		\begin{equation}
			F(\omega) = \int_{-M}^{M} \cos(\frac{\pi}{M}t)\mathrm{e}^{-\mathrm{i}\omega t},\mathrm{d}t
		\end{equation}
		This equation allows us to define our CFT in an interval $\mathbb{M}$, that we choose based on the fractional amount we want to shift, as to alleviate the oscillations that occur with different sizes of M.
		\begin{equation}
		...
		\begin{figure}[h]
			\centering
			\includegraphics[width=0.48\columnwidth]{images/filter_m_1.png} \hfill		\includegraphics[width=0.48\columnwidth]{images/filter_m_1_5.png} \\
			\parbox{0.48\columnwidth}{\centering Filter with M = 1.} \hfill
			\parbox{0.48\columnwidth}{\centering Filter with M = 2.}
			\centering
			\includegraphics[width=0.48\columnwidth]{images/filter_m_2.png} \hfill		\includegraphics[width=0.48\columnwidth]{images/filter_m_3.png} \\
			\parbox{0.48\columnwidth}{\centering Filter with M = 2.} \hfill
			\parbox{0.48\columnwidth}{\centering Filter with M = 3.}
			\caption{Filter}
		\end{figure}
		%
		
		Now that we have our filters we can phase shift them by the fractional amount in the same way that was defined in equation \ref{shift_equation}
